...
//   1. 将完整结果写入 toolResultsDir/{sessionId}/{toolUseId}.txt
...
// 图片类型豁免，直接通过。
//
// Brain 包改造：路径通过 ToolResultStorageConfig 注入，
// 不再依赖 Electron 的 paths 模块。
// ============================================================
...
/** Tool Result Storage 配置 — 路径由调用方注入 */
export interface ToolResultStorageConfig {
  /** 工具结果持久化根目录。桌面端传入 ~/.wzxclaw/tool-results/ */
  baseDir: string
}
...
/**
 * 获取工具结果目录路径。
 * 由注入的 baseDir 和 sessionId 构成。
 */
function getToolResultsDir(config: ToolResultStorageConfig, sessionId: string): string {
  return path.join(config.baseDir, sessionId)
}
...
export async function maybePersistLargeToolResult(
  ...
  const dir = getToolResultsDir(config, sessionId)
  await fs.mkdir(dir, { recursive: true })

  // 文件名：{toolUseId}.txt（toolUseId 已是唯一标识符）
  const fileName = `${sanitizeForFilename(toolUseId)}.txt`
 ...
 * 用于 session 结束后的资源回收。
 */
export async function cleanupToolResults(
  config: ToolResultStorageConfig,
  sessionId: string,
): Promise<void> {
  const dir = getToolResultsDir(config, sessionId)
...
/**
 * 清理所有过期的工具结果目录（baseDir 下 30 天未访问的）。
 * 在应用启动时后台静默执行，不影响主流程。
 */
export async function cleanupExpiredToolResults(config: ToolResultStorageConfig): Promise<void> {
  try {
    const sessionDirs = await fs.readdir(config.baseDir)
    const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000 // 30 days
    for (const dir of sessionDirs) {
      const fullPath = path.join(config.baseDir, dir)
      try {
