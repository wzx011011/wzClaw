...
describe('ContextManager', () => {
  ...
  // ---- Unit 4: Pre-Compact (60% threshold) ----
  describe('shouldPreCompact', () => {
    it('returns false when messages are under 60% threshold', () => {
      const cm = new ContextManager()
      const messages: Message[] = [
        { role: 'user', content: 'Hello', timestamp: Date.now() }
      ]
      expect(cm.shouldPreCompact(messages, 'gpt-4o')).toBe(false)
    })

    it('returns true when messages exceed 60% of context window', () => {
      const cm = new ContextManager()
      // gpt-4o: contextWindow=128000, 60% = 76800 tokens
      const longContent = 'a '.repeat(80000)
      const messages: Message[] = [
        { role: 'user', content: longContent, timestamp: Date.now() }
      ]
      expect(cm.shouldPreCompact(messages, 'gpt-4o')).toBe(true)
    })

    it('returns false when preCompactThreshold is 0 (disabled)', () => {
      const cm = new ContextManager({ preCompactThreshold: 0 })
      const longContent = 'a '.repeat(120000)
      const messages: Message[] = [
        { role: 'user', content: longContent, timestamp: Date.now() }
      ]
    ...
    it('triggers earlier than shouldCompact (60% vs ~93%)', () => {
      const cm = new ContextManager()
      // gpt-4o: 60% = 76800, shouldCompact threshold = 98616
      const content = 'a '.repeat(80000)
      const messages: Message[] = [
        { role: 'user', content: content, timestamp: Date.now() }
      ]
      // Pre-compact triggers (80K > 76.8K)
      expect(cm.shouldPreCompact(messages, 'gpt-4o')).toBe(true)
      // Full compact does NOT trigger yet (80K < 98.6K)
      expect(cm.shouldCompact(messages, 'gpt-4o')).toBe(false)
    })

    it('respects custom preCompactThreshold', () => {
      const cm = new ContextManager({ preCompactThreshold: 0.40 })
      // gpt-4o: 40% = 51200 tokens
      const content50k = 'a '.repeat(50000)
      const messages: Message[] = [
        { role: 'user', content: content50k, timestamp: Date.now() }
      ]
      ...
      const content60k = 'a '.repeat(60000)
      const messages2: Message[] = [
        { role: 'user', content: content60k, timestamp: Date.now() }
      ]
      expect(cm.shouldPreCompact(messages2, 'gpt-4o')).toBe(true)
    })
  })
})
