...
- [ ] **TOOL-02**: FileWrite 工具 — 创建或覆盖文件
- [ ] **TOOL-03**: FileEdit 工具 — 基于搜索替换的精确编辑（old_string → new_string），防止竞态条件
- [ ] **TOOL-04**: Bash 工具 — 执行 shell 命令，支持超时（默认 120s），流式输出 stdout/stderr
- [ ] **TOOL-05**: Grep 工具 — 正则表达式搜索文件内容，返回匹配行
- [ ] **TOOL-06**: Glob 工具 — 按文件名模式搜索文件路径
- [ ] **TOOL-07**: 工具调用可视化 — 在 Chat 中显示 Agent 调用了什么工具，输入参数和输出结果
- [ ] **TOOL-08**: 工具权限系统 — 破坏性操作（文件写入、Bash 执行）需用户确认；只读操作可自动允许
...
- [ ] **EDIT-04**: 打开文件夹作为工作区 — 通过对话框选择项目根目录
