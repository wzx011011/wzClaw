...
The screenshots in the current product flow map to these core capabilities:

### Desktop IDE

| Area | What it shows | Why it matters |
|------|---------------|----------------|
| Workspace Home | Workspace cards, recent sessions, relay connection status, and one-click phone pairing | Start from a project-level dashboard instead of a single chat window |
| Workspace Detail | Bound folders, system prompt entry, session list, and quick entry into the IDE | Keep project context, instructions, and conversation history organized together |
| AI Coding Chat | Streaming agent output, tool-call timeline, context restore, session sidebar, and model selector | Review what the agent searched, edited, and answered while keeping conversations persistent |
| Split IDE Layout | Chat beside the built-in browser/file area with responsive panes | Research, browse, inspect files, and continue the coding conversation in one workspace |
| Plugin Center | Discover/installed/market tabs, URL install entry, and plugin directory hints | Extend the IDE with custom tools, skills, and integrations |
| Mobile Pairing | QR-code/token modal for connecting the Android companion app through the relay | Pair desktop and phone quickly without manual token copying |

### Android Companion

| Area | What it shows | Why it matters |
|------|---------------|----------------|
| Desktop List | Relay status, online desktop cards, and scan-to-add desktop entry | Pick the active desktop from your phone and confirm relay connectivity |
| Settings | Connect/disconnect controls, push notifications, background keep-alive, cache clearing, theme mode, and accent color | Tune mobile reliability and appearance without touching desktop settings |
| Session Drawer | Workspace selector, cached sessions, refresh/new-session actions, and file browsing entry | Switch between desktop sessions remotely and keep recent conversations available |
| Mobile Chat | Streaming conversation view, tool results, session status banner, voice/input controls, and send action | Drive the same coding agent from Android while watching tool execution in real time |
...
当前截图对应的产品能力可以按桌面端和 Android 伴侣端快速理解：

### 桌面端 IDE

| 区域 | 截图体现的功能 | 价值 |
|------|----------------|------|
| 工作区首页 | 工作区卡片、最近会话、Relay 连接状态、一键连接手机 | 从项目级入口开始管理 AI 编程工作，而不是只打开单个聊天窗口 |
| 工作区详情 | 绑定文件夹、系统提示词入口、会话列表、进入工作区按钮 | 把项目路径、指令和历史会话集中在同一个工作区里管理 |
| AI 编程会话 | 流式回复、工具调用时间线、上下文恢复、会话侧栏、模型选择 | 清楚看到 Agent 搜索了什么、调用了哪些工具、输出了什么结论 |
| 分栏工作台 | 聊天区与内置浏览器/文件区并排展示，支持窄屏响应式布局 | 边浏览资料、边查看文件、边继续给 Agent 下达任务 |
| 插件中心 | 发现/已安装/市场页签、URL 安装入口、插件目录提示 | 为 IDE 扩展自定义工具、技能和外部集成能力 |
| 手机配对 | QR Code / Token 连接弹窗 | 通过 Relay 快速把 Android 端和桌面端配对，避免手动复制长 token |

### Android 伴侣端

| 区域 | 截图体现的功能 | 价值 |
|------|----------------|------|
| 桌面端列表 | Relay 已连接、在线桌面卡片、扫码添加桌面 | 在手机上确认桌面在线，并选择要控制的目标桌面 |
| 设置页 | 连接/断开、推送通知、后台保持连接、清空缓存、主题和主题色 | 控制移动端可靠性、通知策略和视觉偏好 |
| 会话抽屉 | 工作区选择、缓存会话、新建/刷新、浏览文件入口 | 远程切换桌面会话，并保留最近会话缓存 |
| 手机聊天页 | 流式消息、工具结果、连接状态条、语音/输入/发送控件 | 在 Android 上直接驱动同一个编码 Agent，并实时观察执行过程 |
