...
8. **拒绝原因带规则名**（2026-09-22 增，对标 Claude Code auto mode 拦截口径）：
   权限拦截/拒绝必须把命中的规则标识透传到手机端——UI 要能显示"为什么被拒"，
   不许只给一个光秃秃的 deny。
9. **仓库配置不可自我提权**（2026-09-22 增，对标 bypassPermissions 项目级设置失效）：
   权限档位与配对凭据只能由用户侧（手机确认/用户级设置）决定；节点上仓库内的
   `.claude/`、`.zcode/` 配置不得越过手机确认自我放行。
10. **首读工作目录外需确认**（2026-09-22 增）：会话首次读取工作目录之外的文件
   前先向用户确认；多目录（/add-dir 类）与 x/* 扩展链路同样适用。
